...
Last updated: 2026-09-22 (v0.4.4 — menu e2e via simulator 0.9.x, CLI 0.1.14 version floor; 389 unit + 104 e2e)
...
| Every picker uses the platform's pixel-matched selectionPrefix — Gin Rummy draw-vs-take, Euchre Order/Pass + trump, Crazy Eights suit all shifted 15px sideways under the cursor on real glasses | unit:launcher (platform) + on-glasses | n/a | n/a | n/a | n/a |
| Crazy Eights suit picker shows the hand you are choosing for — naming a suit is a decision about the cards you hold, and the view used to hide them | unit:crazy8:suitpick-hand | n/a | n/a | n/a | n/a |
| Crazy Eights opponent line carries last action + cards left on ONE line; play view stays inside the 8-10 line budget with a 13-card hand | unit:crazy8:oppline + unit:crazy8:budget | n/a | n/a | n/a | n/a |
| Gin Rummy reports whether the opponent drew blind or TOOK YOUR DISCARD — the most valuable read in the game, and nothing on screen carried it | unit:ginrummy:oppturn + unit:ginrummy:budget | n/a | n/a | n/a | n/a |
| Glasses contextual menu declared on the page container and selections routed to the Runtime; invalid entries dropped before send (firmware rejects the whole page on a bad menu) | unit:runtime (platform) + e2e:menu (every game: `context_menu` → `down` → `click` fires `menuItemClickEvent` itemID 2; `context_menu` → `click` returns to `view=launcher`) | e2e:menu (no console errors after selection) | n/a | n/a | n/a |
| Store listing matches the build: description names every game main.ts registers, tagline states the count, no Solitaire, portal field limits, every cited screenshot exists at 576×288 (the listing said seven games and cited three missing files for months; verified by reintroducing both) | unit:store-listing:6 | unit:store-listing (fails on drift) | n/a | n/a | n/a |
| Toolchain: simulator 0.9.1 + SDK 0.0.14 | e2e:* (80 checks green) | n/a | n/a | n/a | n/a |
