...
| Glasses contextual menu declared on the page container and selections routed to the Runtime; invalid entries dropped before send (firmware rejects the whole page on a bad menu) | unit:runtime (platform) + e2e (page still creates with menu) | e2e (no console errors) | n/a | n/a | sim 0.9.1 drives it: `context_menu` → `down` → `click` fired itemID 2 and re-dealt (verified by hand 2026-09-22; not yet in regression.mjs) |
