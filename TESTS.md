...
| Glasses contextual menu declared on the page container and selections routed to the Runtime; invalid entries dropped before send (firmware rejects the whole page on a bad menu) | unit:runtime (platform) + e2e (page still creates with menu) | e2e (no console errors) | n/a | n/a | **tap-and-hold path is hardware-only — the sim API has no long-press** |
